...
const login = async (req, res, next) => {
    ...
    if (!isValidPassword || !isValidPattern) {
        if (currentAttempts.attempts + 1 === Number(process.env.MAX_ATTEMPTS)) {
            await userAttemptsModel.findOneAndUpdate({username: username}, { $inc: { attempts: 1 }, token: nanoid(32)}).catch(err => console.log(err))
            //console.log("sending email entered")
            sendEmail(currentAttempts.email)
        }
        await userAttemptsModel.findOneAndUpdate({username: username}, { $inc: { attempts: 1 }}).catch(err => console.log(err))
